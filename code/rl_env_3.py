...
class DroneGymEnv(gym.Env):
    def __init__(self, num_drones=30, numberObjects = 1500, bp=0):
        ...
        self.env = Environment(drone_version=3,numberObjects=numberObjects)
    ...
    def step(self, action):
        ...
        dist = 0.0
        for dronnnnnn in self.drones:
            dist_to_lead = self.leader_drone.position - dronnnnnn.position
            dist+= np.linalg.norm(dist_to_lead)
        dist /= len(self.drones)

        truncated = (len(self.drones) < (self.num_drones / 2)) or dist > 50.0
...
if __name__ == "__main__":
    ...
    model.set_env(env)
    model.learn(total_timesteps=104856)
    model.save(("./models/RL_env_3_1_small"))
    ...
    model.set_env(env)
    model.learn(total_timesteps=104856)
    model.save(("./models/RL_env_3_1_big"))
    ...
    model.set_env(env)
    model.learn(total_timesteps=104856)
    model.save(("./models/RL_env_3_1_combi"))
    ...
    # else:
    #     model = PPO.load(("./models/RL_env_3_"+suffix), env, n_steps=n_steps, verbose=1, n_epochs=n_epochs, batch_size=batch_size)

    # model.learn(total_timesteps=total_timesteps)
    # model.save(("./models/RL_env_3_"+suffix))
